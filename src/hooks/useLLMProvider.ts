...
// import { usePerformance, ExtendedPerformanceEntry } from '@/contexts/PerformanceContext'; // Import usePerformance
...
//             logger.error(`[${COMPONENT_ID}][${queryId}] ❌ Error in ${context}: ${error.message}`);
...
//         if (!apiKey) {
//             logger.error(`[${COMPONENT_ID}] ❌ API key is missing. OpenAI client initialization skipped.`);
//             return;
//         }
...
//         ): Promise<void> => {
//             logger.info(`[${COMPONENT_ID}][${queryId}] 🏃‍♂️ Starting thread run with streaming...`);

//             const thread = await openai.beta.threads.create();
//             logger.debug(`[${COMPONENT_ID}][${queryId}] 🧵 Thread created with ID: ${thread.id}`);

//             await openai.beta.threads.messages.create(thread.id, {
//                 role: 'user',
//                 content: formattedMessage,
//             });
//             logger.debug(`[${COMPONENT_ID}][${queryId}] 💬 User message added to thread`);

//             const run = openai.beta.threads.runs.stream(thread.id, {
//                 assistant_id: assistantId,
//             });

//             run.on('textCreated', () => logger.info(`[${COMPONENT_ID}][${queryId}] assistant >`))
...
//                         const measures = performance.getEntriesByName(`MoveToFirstChunk_duration_${queryId}`);
//                         if (measures.length > 0) {
//                             const measure = measures[0];
//                             const duration = measure.duration;

//                             console.log(`[${COMPONENT_ID}][${queryId}] Move to first chunk took ${duration.toFixed(2)}ms`);

//                             logger.performance(`[${COMPONENT_ID}][${queryId}] Move to first chunk took ${duration.toFixed(2)}ms`);
//                             loglog.performance(`Move to first chunk took ${duration.toFixed(2)}ms`, queryId);
...
//                     }
//                 })
//                 .on('toolCallCreated', toolCall => logger.info(`[${COMPONENT_ID}][${queryId}] assistant > ${toolCall.type}\n\n`))
//                 .on('toolCallDelta', toolCallDelta => {
//                     if (toolCallDelta.type === 'code_interpreter' && toolCallDelta.code_interpreter) {
//                         if (toolCallDelta.code_interpreter.input) {
//                             logger.info(`[${COMPONENT_ID}][${queryId}] Code input: ${toolCallDelta.code_interpreter.input}`);
//                             loglog.info(`Code input: ${toolCallDelta.code_interpreter.input}`, queryId);
//                         }
...
//                 });
//             run.on('messageDone', () => {
//                 logger.info(`[${COMPONENT_ID}][${queryId}] 🏁 Response streaming completed.`);
...
//                 console.log(`[${queryId}] Complete Response:`, streamedContentRef.current);
//                 logger.info(`[${COMPONENT_ID}][${queryId}] Complete Response: ${streamedContentRef.current}`);
//                 loglog.info(`Complete Response: ${streamedContentRef.current}`, queryId);
//             }).on('error', error => {
...
//             logger.info(`[${COMPONENT_ID}][${queryId}] 🚀 Starting response generation process`);
//             loglog.info('🚀 Starting response generation process', queryId);

//             try {
//                 const formattedMessage = await createUserMessage(userMessage, roleDescription, state.conversationSummary, goals);
//                 console.log(`[${queryId}] Created User Message:`, userMessage);
//                 loglog.debug(`Created User Message: ${userMessage}`, queryId);

//                 logger.debug(`[${COMPONENT_ID}][${queryId}] 📝 Formatted user message: "${formattedMessage}..."`);
//                 loglog.debug(`Formatted user message: "${formattedMessage}..."`, queryId);
...
//                 performance.measure(`generateResponse_duration_${queryId}`, 'generateResponse_start', 'generateResponse_end');

//                 const measures = performance.getEntriesByName(`generateResponse_duration_${queryId}`);
//                 if (measures.length > 0) {
//                     const measure = measures[0];
//                     // const entry: ExtendedPerformanceEntry = {
...
//                     // };
//                     // addEntry(entry);
//                     logger.performance(`[${COMPONENT_ID}][${queryId}] 🎉 generateResponse took ${measure.duration.toFixed(2)}ms`);
//                     loglog.performance(`generateResponse took ${measure.duration.toFixed(2)}ms`, queryId);
//                 }
//             } catch (error) {
//                 handleError(error, queryId);
...
//             }
//         },
//         [roleDescription, openai, runThread, assistantId, state.conversationSummary, goals]
//     );
...
//             if (!openai) {
//                 logger.error(`[${COMPONENT_ID}] ❌ OpenAI client not initialized. Cannot summarize conversation.`);
//                 return;
//             }
...
//                 logger.info(`[${COMPONENT_ID}] 📝 Conversation summarized successfully.`);
//             } catch (error) {
//                 logger.error(`[${COMPONENT_ID}] ❌ Error summarizing conversation: ${(error as Error).message}`);
//             } finally {
...
//         if (!openai) {
//             logger.error(`[${COMPONENT_ID}] ❌ OpenAI client not initialized. Cannot generate suggestions.`);
//             return;
//         }
...
//         const conversationText = conversationHistory.map(msg => `${msg.type === 'user' ? 'User' : 'Assistant'}: ${msg.content}`).join('\n');
...
//             const suggestionsContent = response.choices[0]?.message.content?.trim() || '';
...
//                 } else {
//                     throw new Error('Parsed suggestions do not match the expected structure.');
//                 }
//             } catch (parseError) {
//                 // Log the parsing error
//                 logger.error(`[${COMPONENT_ID}] ❌ Error parsing suggestions JSON: ${(parseError as Error).message}`);
...
//                     logger.error(
//                         `[${COMPONENT_ID}] ❌ Secondary error parsing suggestions JSON: ${(secondaryParseError as Error).message}`
//                     );
...
//                         if (trimmedLine.startsWith('Can') || trimmedLine.startsWith('Could') || trimmedLine.startsWith('How')) {
...
//         } catch (error) {
//             logger.error(`[${COMPONENT_ID}] ❌ Error generating suggestions: ${(error as Error).message}`);
//         } finally {
